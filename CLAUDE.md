...
- `lib/server.py` — Python HTTP server (stdlib `http.server`), listens on port 8421, routes POST `/telegram/webhook` and POST/GET `/whatsapp/webhook`. Multi-bot dispatch: matches Telegram webhooks via secret-token header, WhatsApp webhooks via HMAC-SHA256 signature verification. Supports dual-platform bots (same bot_id serving both Telegram and WhatsApp). Loads bot registry from `~/.claudio/bots/*/bot.env`. SIGHUP handler for hot-reload. Composite queue keys (`bot_id:chat_id` for Telegram, `bot_id:phone_number` for WhatsApp) for per-bot, per-user message isolation. `/reload` endpoint (requires `MANAGEMENT_SECRET` authentication). Feature flag: `CLAUDIO_PYTHON_HANDLERS=1` env var routes webhooks to the Python handler path instead of Bash.
- **Python webhook handler modules** (used when `CLAUDIO_PYTHON_HANDLERS=1`):
...
- **Bash webhook handlers** (legacy, used when `CLAUDIO_PYTHON_HANDLERS` is unset):
