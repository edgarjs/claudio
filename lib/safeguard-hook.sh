...
#
# Replaces the old lib/safeguards/ PATH wrapper approach. This hook is
...
if printf '%s' "$COMMAND" | grep -qE '(systemctl)\b' && \
   ...
   printf '%s' "$COMMAND" | grep -qE '\bclaudio(\.service)?\b'; then
    ...
    # Audit log
    local_log="${CLAUDIO_PATH:-${HOME:-/tmp}/.claudio}/claudio.log"
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] [safeguard-hook] BLOCKED: $COMMAND" >> "$local_log" 2>/dev/null || true
    exit 0
fi
