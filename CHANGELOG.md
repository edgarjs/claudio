...
- Missing agent token usage: track token usage in agents via `--output-format json` so agent costs appear in `token_usage` table (#33)
- Dead code: remove dead code and redundant `CREATE TABLE` statements (#33)
